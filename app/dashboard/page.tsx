...
export default async function DashboardPage({
    ...
    // Si no hay workspace seleccionado, redirigir a la página de selección
    if (!workspaceId) {
        redirect("/workspaces");
    }
    ...
    if (error || !workspace) {
        return (
            <div className="flex flex-col items-center justify-center h-[50vh] gap-4">
                ...
                <p className="text-muted-foreground">{error || "No pudimos cargar la información del espacio de trabajo."}</p>
                <Button asChild onClick={() => redirect("/dashboard")}>
                    <span className="cursor-pointer">Volver a mis workspaces</span>
                </Button>
            </div>
        );
    }

    // ESTADO 2: WORKSPACE ACTIVO (CON SIDEBAR Y DASHBOARD COMPLETO)
    return (
        <div className="min-h-screen bg-background text-foreground">
            ...
            <div className="flex h-screen pt-16">
                <Sidebar workspaces={allWorkspaces || []} currentWorkspaceId={workspaceId} />
